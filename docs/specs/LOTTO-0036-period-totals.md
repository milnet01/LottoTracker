...
should expect is stated in §4.3: the periods begin where the merged results
record does, not at the first ticket. §4.3 owns that floor and the figure for
it; naming a date here as well is how the two drift apart.
...
`build_model()` gains one key, and **the bucketing itself is a pure function
taking its data sources as arguments**: `period_buckets(all_tickets, wins,
entry_draws, increments)`, where `entry_draws(ticket, plus_flag)` gives the ISO
dates that entry covers, or `None` when nothing can score it, and
`increments(game, era)` gives that era's per-board, per-draw cents.
`build_model()` calls it once, which is one more pass over the tickets than it
made before.

**That seam is load-bearing, not a style preference.** INV-59 requires a fixture
carrying an unresolved ticket that WINS, and INV-60 one whose only draws belong
to an unresolved ticket; the live dump supplies neither. Accumulated inline in
`build_model()`'s own loop, the rules could be reached only by calling
`build_model()` — which needs the dump and the results record — so those two
invariants would have no drivable entry point at all and §7's argument for a
separate verifier would collapse. The injected arguments are what let
`tools/verify_periods.py` drive the real rules over synthetic tickets.

`page.py` renders it verbatim and sums nothing —
...
| §4.5 `page.py` renders `spend_cents` and `won_cents` verbatim and sums no displayed column | **nothing** — code review only. It was a clause of INV-59 until 2026-09-06 and was moved here because every case that invariant names is builder-side: `tools/verify_periods.py` never imports `page`, so a `_periods_section()` that added a totals row summing the spend column would leave all four cases green. Claiming coverage for it was worse than recording that there is none. A case in `tools/verify_page.py` asserting each rendered cell equals the model integer would close it |
...
- **`CLAUDE.md`** — the `serve.py` bullet gains `periods`, and the cardinal-rule
  ...
  cell. **Two counted claims in § Commands move and must be re-measured, not
  adjusted by arithmetic:** "these eight scripts *are* the test suite" goes to
  nine, and the data-dependent group goes from five to six (the CI lane stays at
  three). `tools/verify_periods.py` needs its own entry in that command block,
  naming INV-57..INV-60. The `verify_page.py` line no longer carries an
  invariant range at all — it points at that file's own `CASES` table, having
  gone stale twice — so nothing is to be added to it and the list is not to be
  restored.
...
- **`CLAUDE.md`** again — § Verification says `--list` shows the breaks of
  `verify_page.py` and deliberately states **no count**, so nothing there moves
  and no number is to be written back in. The five new breaks belong to
  `tools/verify_periods.py::BREAKS`.
...
four short cells per row. The bucketing is one more pass over the tickets than
`build_model()` made before — `period_buckets()` is a separate call taking the
data `build_model()` has already gathered (§4.5), not an accumulation folded
into its existing loop. The work is two dictionary increments per covered draw
and no new I/O.
